...
for topic, conf in config.items():
    dfs[topic] = pd.DataFrame(columns=['timestamp', 'datetime'] + conf['values'])
...
for topic, msg, _timestamp in msgs:
    ...
    dfs[topic] = pd.concat([dfs[topic], pd.DataFrame([rowDict])], ignore_index=True)
...
df_camera = dfs[SYNC_TOPIC].sort_index()
df_speed = dfs['/speed'].sort_index()

df_combined = pd.merge_asof(left=df_camera, right=df_speed, left_index=True, right_index=True, direction='nearest')

filepath = os.path.join(output_dir, 'topics_combined.csv')
df_combined.to_csv(filepath, header=True, index=False)
print ("Done!")